 ...
 * 실사용에는 지장이 없다. */
#define BLE_SVC_UUID16            0xFF87    /* 설정/OTA 서비스 */
#define BLE_SVC_UUID16_RX         0xFF88    /* 호스트 -> 보드 */
#define BLE_SVC_UUID16_TX         0xFF89    /* 보드 -> 호스트 */

/* 속성 테이블 안에서의 위치. 콜백이 어느 특성인지 구분하는 데 쓴다. */
#define BLE_SVC_IDX_RX_VALUE      2
#define BLE_SVC_IDX_TX_VALUE      4
#define BLE_SVC_IDX_TX_CCCD       5


static T_SERVER_ID    svc_id      = 0xFF;
static bool           is_notify_on = false;
static ble_svc_rx_cb  rx_handler  = NULL;
...
static const T_ATTRIB_APPL ble_svc_tbl[] =
{
  ...
  /* RX 값 */
  {
    ...
    { LO_WORD(BLE_SVC_UUID16_RX), HI_WORD(BLE_SVC_UUID16_RX) },
    0,
    NULL,
    GATT_PERM_WRITE
  },
  ...
  {
    ATTRIB_FLAG_VALUE_APPL,
    { LO_WORD(BLE_SVC_UUID16_TX), HI_WORD(BLE_SVC_UUID16_TX) },
    0,
    NULL,
    GATT_PERM_NONE
  },
  /* TX 의 CCCD. 호스트가 여기에 써야 notify 가 시작된다. */
  {
...
{
  if (svc_id == 0xFF || is_notify_on == false) return false;

  return server_send_data(conn_id, svc_id, BLE_SVC_IDX_TX_VALUE,
                                    ...
                                    P_FUN_WRITE_IND_POST_PROC *p_post_proc)
{
  ...
  *p_post_proc = NULL;
  ...
  if (attrib_index != BLE_SVC_IDX_RX_VALUE) return APP_RESULT_ATTR_NOT_FOUND;
  if (p_value == NULL)                      return APP_RESULT_INVALID_VALUE_SIZE;

  if (rx_handler != NULL)
  {
    rx_handler(conn_id, p_value, length);
  }
                             ...
                             uint16_t index, uint16_t cccbits)
{
  ...
  if (index == BLE_SVC_IDX_TX_CCCD)
  {
    is_notify_on = (cccbits & GATT_CLIENT_CHAR_CONFIG_NOTIFY) ? true : false;
  }
}
